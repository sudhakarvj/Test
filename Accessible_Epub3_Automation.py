...
<li><a epub:type="index" href="xhtml/index.xhtml">Index</a></li>
    ...
    if xhtml_file:
        for filename in xhtml_file:
            ...
            xhtml_cnt = re.sub(r'(<p class="(?:bq|block|blockquote)"(?: [^>]*)?>(?:(?!</p>).)*</p>)', r'<blockquote>\n\g<1>\n</blockquote>', xhtml_cnt, 0, re.I | re.S)
            xhtml_cnt = re.sub(r'<sup>\s*(<a(?: [^>]*)?) (href="notes\.x?html[^"]*"(?: [^>]*)?>)((?:(?!</a>).)*)</a>\s*</sup>', r'\g<1>/><a epub:type="noteref" role="doc-noteref" \g<2><sup class="sup">\g<3></sup></a>', xhtml_cnt, 0, re.I | re.S)
            xhtml_cnt = re.sub(r'(<a(?: [^>]*)?) (href="notes\.x?html[^"]*"(?: [^>]*)?>)\s*<sup>((?:(?!</sup>).)*)</sup>\s*</a>', r'\g<1>/><a epub:type="noteref" role="doc-noteref" \g<2><sup class="sup">\g<3></sup></a>', xhtml_cnt, 0, re.I | re.S)
            xhtml_cnt = re.sub(r'\.html', r'.xhtml', xhtml_cnt, 0, re.I | re.S)

            # All Page
            if not re.search(r'(cover|toc|contents?)\.xhtml', filename, re.I | re.S):
                ...
                xhtml_cnt = re.sub(r'(<section epub:type="title">.*?<h\d[^>]*?)\s+id="[^"]*"([^>]*>)', update_id, xhtml_cnt, flags=re.I | re.S)
                xhtml_cnt = re.sub(r'<p class="bib"(?: [^>]*)?>((?:(?!</p>).)*)</p>', r'<li class="bib">\g<1></li>', xhtml_cnt, 0, re.I | re.S)
                xhtml_cnt = re.sub(r'(<h\d+(?: [^>]*)?>(?:(?!</h\d+>).)*</h\d+>\s*)(<li class="bib">)',r'\g<1><ul class="biblist">\n\g<2>', xhtml_cnt, 0, re.I | re.S)
                xhtml_cnt = re.sub(r'(<section(?: [^>]*)?>\s*<h\d+(?: [^>]*)?>(?:(?!</h\d+>).)*</h\d+>\s*<ul class="biblist">(?:(?!</section>).)*)(</section>)',
                    r'\g<1></ul>\n\g<2>', xhtml_cnt, 0, re.I | re.S)

            # Chapter files
            if re.search(r'ch(?:apter)?\d+\.xhtml', filename, re.I | re.S):
                # Step 1: Title case function (same as you wrote)
                def title_case(txt):
                    if txt.isupper():
                ...
                # Step 2: Track processed classes
                modified_classes = set()

                # Step 3: Replace headings and collect class names
                def transform_heading(m):
                    ...
                    if class_match:
                        ...
                        for cls in class_names:
                            if cls.startswith('h') and cls not in modified_classes:
                                modified_classes.add(cls)
                    return open_tag + title_case(text) + close_tag
                ...
                # Step 4: Update style.css with 'text-transform:uppercase;' for modified classes
                def add_text_transform(css_file, modified_classes):
                    css_cnt = _open_utf8(css_file)

                    for cls in modified_classes:
                        # Match .classname { ... } with content inside braces
                        pattern = rf'(\.{re.escape(cls)}\s*\{{)(.*?)(\}})'

                        def repl(m):
                            opening, content, closing = m.group(1), m.group(2), m.group(3)
                            if 'text-transform' not in content.lower():
                                content = content.rstrip() + '\n\ttext-transform:uppercase;'
                            return opening + content + closing

                        css_cnt = re.sub(pattern, repl, css_cnt, count=1, flags=re.I | re.S)

                    _save_utf8(css_file, css_cnt)

                # Apply to all modified classes
                add_text_transform(stylesheet_css, modified_classes)
            ...
            if re.search(r'index\.xhtml', filename, re.I | re.S):
                ...
                xhtml_cnt = re.sub(r'</a></a>', r'', xhtml_cnt, 0, re.I | re.S)
        ...
        if nav_xhtml:
                ...
                for file in re.finditer(r'<a href="(?:xhtml/)?([^"]*)"', nav_list, re.I | re.S):
                    ...
                    if xhtml:
                        ...
                        for pg in re.finditer(r'<span id="(pg_([^"]*))"(?: [^>]*)?/?>', xhtml_cnt, re.I | re.S):
                            nav_page_list += f'<li><a href="xhtml/{base}#{pg.group(1)}">{pg.group(2)}</a></li>\n'

            nav_cnt = re.sub(r'(<nav aria-labelledby="h-landmarks" epub:type="landmarks">(?:(?!</nav>).)*</nav>)', rf'\g<1>\n<nav epub:type="page-list" class="hidden" role="doc-pagelist" aria-labelledby="ric-pg-list">\n<h1 epub:type="title" class="title" id="ric-pg-list">Page List</h1>\n<ol class="none">\n{nav_page_list}\n</ol>\n</nav>', nav_cnt, 1, re.I | re.S)
            ...
            xhtml = _get_file_list(OutputDir, 1, 1, '.xhtml$')
            a_list = ''
            if xhtml:
                xhtml_path = dirname(xhtml[0])
                nav_path = os.path.join(xhtml_path, 'nav.xhtml')
                toc = _get_file_list(OutputDir, 1, 1, '(toc|contents?).xhtml$')
                if toc:
                    toc_cnt = _open_utf8(toc[0])
                    for a in re.finditer(r'<a(?: [^>]*)?href="([^"]*)"(?: [^>]*)?>((?:(?!</a>).)*)</a>', toc_cnt, re.I | re.S):
                        a_list += f'<li><a href="{a.group(1)}">{a.group(2)}</a></li>\n'
                a_list = a_list.rstrip('\n')
                a_list = re.sub(r'(<li><a href="part[^"]*"(?: [^>]*)?>((?:(?!</li>).)*))</li>', r'\g<1>\n<ol class="none">', a_list, 0, re.I | re.S)
                a_list = re.sub(r'(<li><a href="part[^"]*"(?: [^>]*)?>)', r'</ol>\n</li>\n\g<1>', a_list, 0, re.I | re.S)
                nav_cnt = f'''<!DOCTYPE html>
...
<body>
<nav aria-labelledby="toc" epub:type="toc" role="doc-toc">
<h2>Contents</h2>
<ol class="none">
...
<nav aria-labelledby="h-landmarks" epub:type="landmarks">
<h2>Guide</h2>
{landmark_cnt}
</nav>
</body>
                ...
                nav_list = re.search(
                    r'<nav aria-labelledby="toc" epub:type="toc" role="doc-toc"(?: [^>]*)?>((?:(?!</nav>).)*)</nav>', nav_cnt,
                    re.I | re.S)
                nav_page_list = ''
                if nav_list:
                    nav_list = nav_list.group()
                    for file in re.finditer(r'<a href="([^"]+)"(?: [^>]*)?>', nav_list, re.I | re.S):
                        file_name = re.search(r'([a-z0-9]+\.xhtml)', file.group(1), re.I | re.S)
                        if file_name:
                            xhtml = _get_file_list(content_dir, 1, 1, file_name.group(1))
                            if xhtml:
                                xhtml_cnt = _open_utf8(xhtml[0])
                                base = basename(xhtml[0])
                                for pg in re.finditer(r'<span id="(pg_([^"]*))"(?: [^>]*)?/?>', xhtml_cnt, re.I | re.S):
                                    nav_page_list += f'<li><a href="{base}#{pg.group(1)}">{pg.group(2)}</a></li>\n'
                    nav_page_list = nav_page_list.rstrip('\n')
                    nav_cnt = re.sub(r'(<nav aria-labelledby="h-landmarks" epub:type="landmarks">(?:(?!</nav>).)*</nav>)',
                                     rf'\g<1>\n<nav epub:type="page-list" class="hidden" role="doc-pagelist" aria-labelledby="ric-pg-list">\n<h1 epub:type="title" class="title" id="ric-pg-list">Page List</h1>\n<ol class="none">\n{nav_page_list}\n</ol>\n</nav>',
                                     nav_cnt, 1, re.I | re.S)

                def sub_level_title(txt):
                    final_cnt = ''
                    for li in re.finditer(r'<li><a href="(ch(?:apter)?\d+\.xhtml)">(?:(?!</a>).)*</a></li>', txt,
                                          re.I | re.S):
                        chapter = _get_file_list(OutputDir, 1, 1, li.group(1))
                        if chapter:
                            chap_cnt = _open_utf8(chapter[0])
                            ...
                            if title_cnt:
                                for idx, heading in enumerate(
                                        re.finditer(r'<(?:h2|h3)(?: [^>]*)?>((?:(?!</(?:h2|h3)>).)*)</(?:h2|h3)>', title_cnt.group(),
                                                    re.I | re.S), start=1):
                                    anchor = f'#lev{idx:02d}'
                                    final_cnt += f'<li><a href="{li.group(1)}{anchor}">{heading.group(1)}</a></li>\n'
                                final_cnt = final_cnt.rstrip('\n')
                                txt = re.sub(r'(<li><a href="ch(?:apter)?\d+\.xhtml">(?:(?!</a>).)*</a>)</li>',
                                             rf'\g<1>\n<ol class="none">\n{final_cnt}\n</ol>\n</li>', txt, 1, re.I | re.S)
                                final_cnt = ''
                    return txt

                nav_cnt = re.sub(r'<nav aria-labelledby="toc" epub:type="toc" role="doc-toc">(?:(?!</nav>).)*</nav>',
                                 lambda m: sub_level_title(m.group()), nav_cnt, 1, re.I | re.S)

                _save_utf8(nav_path, nav_cnt)
        ...
        if pack_opf:
            ...
            opf_cnt = re.sub(r'<dc:creator(?: [^>]*)?>(((?!</dc:creator>).)*)</dc:creator>', r'<dc:creator id="creator1">\g<1></dc:creator>\n<meta refines="#creator1" property="role" scheme="marc:relators">aut</meta>\n<meta refines="#creator1" property="file-as">\g<1></meta>', opf_cnt, 1, re.I | re.S)
        ...
        if ncx_file:
            ...
            ncx_cnt = re.sub(r'<ncx(?: [^>]*)?>', lambda m: re.sub(r' xml:lang="[^"]+"', ' xml:lang="en-US"', m.group(), 0, re.I|re.S), ncx_cnt, 0, re.I | re.S)
            ncx_cnt = re.sub(r'<navPoint(?: [^>]*)?>((?!</navPoint>).)*</navPoint>', lambda m: re.sub(r'\n', '', m.group(), 0, re.I|re.S), ncx_cnt, 0, re.I | re.S)
            last_id = re.search(r'<navPoint(?: [^>]*)?playOrder="([^"]+)">((?!</navPoint>).)*</navPoint>\s*</navMap>', ncx_cnt, re.I | re.S)
            if last_id:
            ...
            if nav_xhtml:
                nav_cnt = _open_utf8(nav_xhtml[0])
                ch_list_cnt = re.search(r'<nav epub:type="page-list" class="hidden" role="doc-pagelist" aria-labelledby="ric-pg-list">(((?!</nav>).)*)</nav>', nav_cnt, re.I | re.S)
                if ch_list_cnt:
                    ch_list_cnt = ch_list_cnt.group(1)
                    for a in re.finditer(r'<a href="([^"]+)">(((?!</a>).)*)</a>', ch_list_cnt, re.I | re.S):
                        last_id += 1
                        page_list += f'<pageTarget id="i{last_id}" playOrder="{last_id}" type="front" value="{a.group(2)}"><navLabel><text>{a.group(2)}</text></navLabel><content src="xhtml/{a.group(1)}"/></pageTarget>\n'
            page_list = page_list.rstrip('\n')
            ncx_cnt = re.sub(r'</navMap>', rf'<pageList>\n{page_list}\n</pageList>\n</navMap>', ncx_cnt, 1, re.I | re.S)
            _save_utf8(ncx_file[0], ncx_cnt)
